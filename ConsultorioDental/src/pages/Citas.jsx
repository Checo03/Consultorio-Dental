...
export default function Citas() {
    ...
    // --- ESTADOS PARA GESTIÓN DE TICKETS ---
    ...
    const [showNuevoPagoInput, setShowNuevoPagoInput] = useState(false); 
    ...
    const handleTicketFormChange = (e) => {
        ...
        let newValue = value;
        if ((name === 'Monto' || name === 'NuevoPago') && value !== '') {
        ...
        setTicketForm(prev => ({ ...prev, [name]: newValue }));
    };
    ...
    const handleCloseTicket = () => {
        ...
        setTicketForm({ Monto: '', Concepto: '', NuevoPago: '' }); 
        setShowNuevoPagoInput(false); 
    };

    const fetchTicket = async (cita) => {
        ...
        try {
            ...
            if (err.response && err.response.status === 404) {
                ...
                alert("Error grave al cargar los datos del ticket.");
            }
        } finally {
    ...
    // Habilita el campo NuevoPago
    const handleAgregarPagoClick = () => {
        setShowNuevoPagoInput(true);
        setTicketForm(prev => ({ ...prev, NuevoPago: '' }));
    };

    const handleTicketSubmit = async (e) => {
        e.preventDefault();

        const isUpdate = ticketCargado && ticketCargado.ID;
        const endpoint = isUpdate ? 'actualizarTicket' : 'crearTicket'; 
        const action = isUpdate ? 'actualiza' : 'guarda';
        
        let dataToSend = {
            IDPaciente: selectedCita.IDPaciente,
            IDCita: selectedCita.ID, 
            Concepto: ticketForm.Concepto,
        };

        if (isUpdate) {
            dataToSend.IDTicket = ticketCargado.ID;
            
            const nuevoPagoValor = (showNuevoPagoInput && ticketForm.NuevoPago)
                ? parseFloat(ticketForm.NuevoPago) : 0;
            
            dataToSend.NuevoPago = nuevoPagoValor;
            
            
        } else {
            dataToSend.Monto = parseFloat(ticketForm.Monto) || 0;
        }

        try {
            await axios.post(`${API_BASE_URL}/${endpoint}`, dataToSend);
            
            alert(`Ticket ${action}do exitosamente.`);
            
            fetchTicket(selectedCita);
            setShowNuevoPagoInput(false);
            setTicketForm(prev => ({ ...prev, NuevoPago: '' }));
            ...
            console.error(`Error al ${action} ticket:`, err.response?.data || err.message);
            alert(`Error al ${action} el ticket.`);
        }
    };
    const getTodayDate = () => {
    ...
    return (
        <div className="citas-container">
            ...
            <main className="citas-main">
                                ...
                                {showTratamientoOptions && (
                                    <div className="citas-card-actions" style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                                        ...
                                        {!citaTieneTratamiento && (
                                            ...
                                            >
                                                <span style={{ fontSize: '13px'}}>Agregar Tratamiento</span>
                                            </button>
                                        )}
                                        ...
                                        {citaTieneTratamiento && (
                                            ...
                                            >
                                                <span style={{ fontSize: '13px'}}> Ver Tratamiento</span>
                                            </button>
                                        )}
                                        
                                        <button
                                            className="citas-action-btn"
                                            onClick={() => handleVerTicket(cita)} 
                                            style={{ flex: 1, backgroundColor: '#75a1ecff', color: 'white' }} 
                                        >
                                            <span style={{ fontSize: '13px'}}>Ver Ticket</span>
                                        </button>
                                    </div>
                                )}
                            </div>
                ...
                {/* --- Modal de Gestión de Tratamiento --- */}
                    ...
                    <div className="citas-modal-overlay" onClick={(e) => e.target === e.currentTarget && handleCloseTratamiento()}>
                        <div className="citas-modal-form large-modal-size">
                            <div className="citas-modal-header">
                                ...
                                <p className="citas-modal-subtitle">Paciente: {selectedCita.Paciente || `ID: ${selectedCita.IDPaciente}`}</p>
                                <button 
                                    className="citas-modal-close"
                            ...
                            {cargandoTratamiento ? (
                                ...
                                <form onSubmit={handleTratamientoSubmit}>
                                    <div style={{ padding: '20px' }}>
                                        ...
                                        {tratamientoCargado && (
                                            <p style={{ fontSize: '0.95rem', marginTop: '10px', color: '#0d0d0dff' }}>
                ...
                {showTicketModal && selectedCita && (
                    
                    <div className="citas-modal-overlay" onClick={(e) => e.target === e.currentTarget && handleCloseTicket()}>
                        <div className="citas-modal-form medium-modal-size">
                            <div className="citas-modal-header">
                                <h2 className="citas-modal-title">
                                    Gestión de Ticket de Pago
                                </h2>
                                <p className="citas-modal-subtitle">Paciente: {selectedCita.Paciente || `ID: ${selectedCita.IDPaciente}`}</p>
                                <button 
                                    className="citas-modal-close"
                            ...
                            {cargandoTicket ? (
                                <div style={{ padding: '40px', textAlign: 'center' }}>Cargando datos del ticket...</div>
                            ) : (
                                <form onSubmit={handleTicketSubmit}>
                                    <div style={{ padding: '20px' }}>
                                        <h3 style={{ marginBottom: '15px', fontSize: '1.2rem', color: '#635e5eff' }}>
                                            {ticketCargado ? 
                                                `Informacion del Ticket` : 
                                                'Crear Nuevo Ticket de Pago'}
                                        </h3>
                                        ...
                                        {ticketCargado && (
                                            <div className="citas-form-group full-width">
                                                <label className="citas-form-label">
                                                    ...
                                                    Fecha de primer pago: <span className="citas-form-info" style={{color:'black'}}>{formatDate(ticketCargado.FechaPago)}</span>
                                                </label>
                                                
                                            </div>
                                        )}
                                        
                                        <div className={`citas-form-grid ${!ticketCargado || showNuevoPagoInput ? 'full-width-hack' : ''}`}>
                                            ...
                                            <div className="citas-form-group">
                                                ...
                                                <input 
                                                    ...
                                                    inputMode="decimal"
                                                    name="Monto" 
                                                    value={ticketForm.Monto} 
                                                    onChange={handleTicketFormChange} 
                                                    placeholder="Ej: 150.00"
                                                    required
                                                    disabled={ticketCargado && showNuevoPagoInput} 
                                                />
                                            </div>

                                            {ticketCargado && (
                                                <div className="citas-form-group">
                                                    <label className="citas-form-label" style={{ color: '#EF4444' }}>
                                                        <svg className="citas-label-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m0 0h6m-6 0v2m7-12h2a2 2 0 012 2v5m-4-5m1 14l-7-7m7 7l-7-7" />
                                                        </svg>
                                                        Saldo Pendiente ($)
                                                    </label>
                                                    <span className="citas-form-info" style={{ 
                                                        color: '#EF4444', 
                                                        fontWeight: 'bold',
                                                        fontSize: '1.2rem',
                                                        display: 'block', 
                                                        ...
                                                        {ticketCargado.Saldo ? parseFloat(ticketCargado.Saldo).toFixed(2) : '0.00'}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                                        
                                        {ticketCargado && showNuevoPagoInput && (
                                            <div className="citas-form-group full-width">
                                                <label className="citas-form-label" style={{ color: '#059669' }}>
                                                    <svg className="citas-label-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div className={`citas-form-group full-width`}>
                                            ...
                                            <textarea 
                                                className="citas-form-textarea"
                                                name="Concepto" 
                                                value={ticketForm.Concepto} 
                                                onChange={handleTicketFormChange} 
                                                placeholder="Descripción del servicio o tratamiento cobrado..."
                                                rows="3" 
                                                required
                                            />
                                        </div>
                                    ...
                                    <div className="citas-form-buttons">
                                        ...
                                        {ticketCargado && !showNuevoPagoInput && (
                                            <button 
                                                type="button" 
                                                className="citas-save-button"
                                                onClick={handleAgregarPagoClick}
                                        ...
                                        <button 
                                            ...
                                            className="citas-save-button"
                                            disabled={
                                                (ticketCargado && showNuevoPagoInput && (!ticketForm.NuevoPago || parseFloat(ticketForm.NuevoPago) <= 0)) ||
                                                (!ticketCargado && (!ticketForm.Monto || !ticketForm.Concepto))
                                            }
                                        >
                                            <svg className="citas-btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                            ...
                                            {ticketCargado 
                                                ? (showNuevoPagoInput ? 'Confirmar Pago' : 'Actualizar Ticket') 
                                                : 'Generar Ticket'}
                                        </button>
                                    </div>
                                </form>
